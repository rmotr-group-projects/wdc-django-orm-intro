...
def task_1_artists_filter_by_popularity():
    ...
    return Artist.objects.filter(popularity__gte=80)

def task_2_artists_get_by_artistic_name():
    """Should return the artist which artistic name is Jimi Hendrix"""
    return Artist.objects.filter(artistic_name='Jimi Hendrix')
...
def task_4_artists_create_song():
    ...
    Song.objects.create(artist_id=100,title="BB's Song", album_name="B.B. King Hits")
...
def task_6_song_edit_album():
    ...
    album_update = Song.objects.filter(title='Superstition')
    album_update.album = "any other name"
    album_update.save
