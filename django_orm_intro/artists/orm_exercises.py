...
def task_4_artists_create_song():
    ...
    return Song(artist_id = Artist.objects.get(artistic_name = 'B.B. King').id).save()
...
def task_6_song_edit_album():
    ...
    new_song = Song.objects.get(title = 'Superstition')
    new_song.album_name = 'Songs About Jane'
    return new_song.save()
